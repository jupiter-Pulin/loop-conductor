...
// verifier 对着被无关提交污染的 diff 照样 8/8 pass（假绿由 codex/reviewer 双影子抓获）。
// 本测试在修复前必失败（worktree 会落在 HEAD=other-branch 上，marker 文件缺失）。
...
test('HEAD ≠ baseBranch 时 worktree 建自 baseBranch（F18 回归钉死）', (t) => {
  ...
  env.writeTask(id, { baseBranch: 'feat/base' });
  env.setScenario([
    { actions: [{ type: 'writeFile', path: 'lib/stats.mjs', content: FIXED_STATS }], session_id: 'sess-m1', cost: 0.1, result: 'r1' },
    verifierStep(1),
  ]);
