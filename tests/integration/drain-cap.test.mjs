...
// 用 maxStepsPerTask=1 制造耗尽：bugfix 需要 2 个变化步（READY→VERIFY，VERIFY→AWAIT_HUMAN_MERGE）。
...
test('步数上限耗尽：显式日志 + 状态落盘 + 下次 run 续推', (t) => {
  const env = makeEnv(t, { config: { maxStepsPerTask: 1 } });
...
test('未耗尽时不打耗尽日志', (t) => {
  ...
  const run = env.run('run'); // 默认 maxStepsPerTask=20，2 步走完后自然 drain 干净
...
test('maxDrainSteps 废弃：告警一次并忽略', (t) => {
  ...
  assert.equal(env.findTask(id).runtime.stage, 'AWAIT_HUMAN_MERGE', '废弃键被忽略，不限制 step');
});
