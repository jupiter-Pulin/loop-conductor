...
// maker 在 worktree 写 .loop-conductor/ac-tests.json（AC→测试映射，harness exclude 覆盖），
...
test('映射有效：逐 AC 定向探测 falsifies/guard_holds/unmapped，映射不进 diff 不进 merge（AC-004/007/008/012/013）', (t) => {
  ...
  env.setScenario([
    { // maker r1：修 bug + 写守卫测试 + 写 AC→测试映射（不可自动化的 AC-003 不写条目）
      actions: [
        ...
        {
          type: 'writeFile',
          path: '.loop-conductor/ac-tests.json',
          content: mappingJson([
  ...
  assert.ok(env.exists(path.join(wt, '.loop-conductor', 'ac-tests.json')), '映射文件物理存在');
  ...
  assert.ok(!names.some((n) => n.startsWith('.loop-conductor/')), '映射文件不得进 diff');

  // AC-007：conductor merge 后 target 主分支无 .loop-conductor/
  const merge = env.run('merge', id);
  assert.equal(merge.status, 0, merge.stderr);
  assert.equal(env.findTask(id).box, 'done');
  assert.ok(!env.exists(path.join(env.targetDir, '.loop-conductor')), 'merge 后主分支不得出现 .loop-conductor/');
