...
test('AC-021: maker spawn 撞限额 → FAILED_BOX(rate_limited) + runtime.rate_limit + 事件', (t) => {
  ...
  assert.equal(after.runtime.rate_limit.resume_stage, 'READY', 'resume_stage = 命中时所在 stage');
  assert.ok(Number.isFinite(after.runtime.rate_limit.hit_at));
  assert.equal(env.calls().length, 1, '零重试：限额只 spawn 一次');
  ...
  assert.equal(rl.resume_stage, 'READY');
});
...
test('AC-021: 同一次 run 内其余任务不再 spawn，stage 不变并记 spawn skipped', (t) => {
  ...
  env.writeTask(hit, { stage: 'READY' });
  env.writeTask(spared, { stage: 'READY' });
  const resetsAt = nowSec() + 2 * HOUR;
  ...
  assert.equal(other.runtime.stage, 'READY', 'stage 不变');
...
test('AC-022: resets_at 已过 → 回 resume_stage，current_round 不变、无产物归档', (t) => {
  const env = makeEnv(t);
  const id = 'task-20260830-405';
  env.writeTask(id, { stage: 'FIXING', miss: 1, currentRound: 2, sessionId: 'sess-keep' });
  const resetsAt = nowSec() - HOUR; // 已过
  env.setScenario([rateLimitStep(resetsAt, { type: 'weekly' })]);
  assert.equal(env.run('run').status, 0);

  const boxed = env.findTask(id);
  assert.equal(boxed.box, 'failed');
  assert.equal(boxed.runtime.rate_limit.resume_stage, 'FIXING');
  const streamBefore = env.dossier(id, 'maker-r2.stream.jsonl');
  ...
  const after = env.findTask(id);
  assert.equal(after.box, 'queue');
  assert.equal(after.runtime.stage, 'FIXING', '回到命中时所在 stage');
  assert.equal(after.runtime.current_round, 2, 'current_round 不变');
  assert.equal(after.runtime.maker_miss_count, 1, '不重置任何计数');
  assert.equal(after.runtime.maker_session_id, boxed.runtime.maker_session_id, '会话不清空');
...
test('AC-022: retry --rate-limited 批量恢复全部此类任务，不碰其他失败类型', (t) => {
  ...
  env.writeTask(a, { stage: 'READY' });
  env.writeTask(b, { stage: 'READY' });
  env.writeTask(other, { stage: 'READY', spent: 99 }); // 预算耗尽 → budget_exceeded
  ...
  env.setScenario([]);
  assert.equal(env.run('run').status, 0);
  assert.equal(env.findTask(other).runtime.last_failure_type, 'budget_exceeded');
