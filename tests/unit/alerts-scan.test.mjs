...
test('scanAlerts：五条规则触发、severity 与 @ 标记、排序', (t) => {
  ...
  const failed = alerts.find((a) => a.severity === 'error');
  assert.ok(failed && failed.mention === true && failed.text.includes('budget_exceeded'), 'failed 箱 error + @');
  ...
  assert.equal(entered.length, 2, 'AWAIT 进入事件各报一次');
  ...
  assert.match(text, /^🔔 loop-conductor 告警（6 条） <@U999>/);
