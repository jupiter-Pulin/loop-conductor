...
// 任务目录布局 state/<box>/<id>/{task.json,runtime.json}；router 纪元的剧本片段在 router-env.mjs。
// 这里只保留通用面：writeRouterTask 造新纪元任务，writeTask 造遗留任务（AC-029 的只读渲染夹具）。
...
export function makeEnv(t, {
  ...
  const api = {
    ...
    /**
     * 直接落盘一个**遗留形态**的任务（旧 stage 名 + 旧计数字段）。
     * 唯一用途是 AC-029 的只读夹具：dashboard 与 dossier-stats 必须照常渲染 P3 之前的任务，
     * 而 CLI 的每个动词都必须拒绝操作它们。新纪元任务用 writeRouterTask。
     */
    writeTask(id, {
      kind = 'bugfix',
      stage = 'READY',
      ...
      fs.writeFileSync(path.join(dir, 'task.json'), `${JSON.stringify({
        schema_version: 1, id, kind, title: 'median 偶数分支返回错误', repo: path.basename(targetRepo),
        targetRepo, baseBranch, testCommand, created_at: '2026-06-11T00:00:00.000Z',
      }, null, 2)}\n`);
      fs.writeFileSync(path.join(dir, 'runtime.json'), `${JSON.stringify({
        schema_version: 1, stage, maker_miss_count: miss, verifier_invalid_count: 0,
        spent_usd: spent, approval: null, current_round: currentRound,
        last_failure_type: lastFailureType, updated_at: '2026-06-11T00:00:00.000Z',
      }, null, 2)}\n`);
      return dir;
    },
