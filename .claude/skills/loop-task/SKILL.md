...
Loop Conductor 是本机的 agent 开发状态机，仓库在 `/Users/jupiter/ai-experiment/loop-conductor`（下称 `$CONDUCTOR_ROOT`）。角色分离（spec→审→实现→冷验收→合并文案）、状态流、命令、数据位置的**唯一事实源是 `$CONDUCTOR_ROOT/README.md`**，本 skill 不复述。你只需要记住闸门：人审共三处——setup profile、spec（或 feasibility option）、merge——全部在 dashboard 里点，不在你手里。
...
1. **前置自检**（一次即可）：
   ...
   ```bash
   CONDUCTOR_ROOT=/Users/jupiter/ai-experiment/loop-conductor
   eval "$(bash "$CONDUCTOR_ROOT/.claude/skills/loop-task/scripts/precheck.sh")"
   ```

   非零退出说明环境不可用，把 stderr 原因报给用户，停。

2. **蒸馏 brief** 写到临时文件，结构：`# 标题` + `## 背景` / `## 问题 / 目标` / `## 非目标` / `## 约束` / `## 验收线索` / `## 开放问题`（每条开放问题必须带 safe default）。**全文贴给用户过目再继续**；brief 忠实记录讨论即可，纠偏交给下游闸门。
