...
8. **`.loop-conductor/ac-tests.json` 每条 AC 都尽量给映射**：`ac_id` 必须逐字匹配 spec 枚举，`expect` 只有 `fail_on_baseline`（新行为）/ `pass_on_baseline`（回归守卫）两个值。映射缺失不 block，但 verifier 会对无机械证明的 AC 从严审计。
