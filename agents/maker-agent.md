...
产出：直接修改当前 worktree 的代码。确保配置的 `testCommand` 全绿——conductor 会亲自复跑（green gate），不采信口头汇报。`testCommand` 与构建命令各自全绿一次后即可收工：其后没有再改过代码就不要做收工前的全量重跑（对 harness-excluded 的 `.loop-conductor/ac-tests.json` 的写入不算改代码），green gate 的机械复跑就是终审。green gate 之后 conductor 还会跑 test gate：把你改动的测试文件叠加回改动前基线（`baseBranch`），映射有效时按 `.loop-conductor/ac-tests.json` 逐条定向复跑（`fail_on_baseline` 基线必须红、`pass_on_baseline` 基线必须绿），映射缺失/非法时降级为复跑整个测试命令。若新行为测试在基线上仍全绿则判为空转并退回修复。因此为新行为写的测试必须在旧代码上失败。
